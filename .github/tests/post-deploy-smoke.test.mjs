...
async function scenario({ commit = sha, proxy = true, css = true } = {}) {
  ...
  const server = createServer((request, response) => {
    ...
    if (path === '/version') {
      ...
      response.end(JSON.stringify({ service: 'atlas-admin-portal', version: '0.1.0', commit }));
    } else if (path === '/') {
      response.end('<html>home</html>');
    } else if (path === '/legal') {
      response.end('<html>legal</html>');
    } else if (path === '/internal/login') {
      ...
      response.setHeader('content-type', 'application/json');
      response.end(JSON.stringify({ data: { service: 'atlas-backend' } }));
    } else {
...
test('comprueba ruta pública, login, JS, CSS, proxy y SHA servido', async () => {
  ...
  for (const path of ['/', '/legal', '/internal/login', '/_next/static/chunks/app.js', '/_next/static/css/app.css', '/api/v1/health']) {
