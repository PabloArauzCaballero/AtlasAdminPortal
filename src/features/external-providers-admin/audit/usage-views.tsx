...
import { Ban, Coins, Database, PhoneCall, Play, TriangleAlert } from "lucide-react";
...
function SuccessBar({ rate }: Readonly<{ rate: number | null }>) {
  if (rate === null) return <span className="text-atlas-muted">Sin llamadas</span>;
  const tone = rate >= 95 ? "bg-emerald-500" : rate >= 80 ? "bg-amber-500" : "bg-red-500";
  return (
...
export function SlaReportView({ query }: Readonly<{ query: Parameters<typeof ReportShell<SlaReport>>[0]["query"] }>) {
  return (
    <ReportShell
      query={query}
    ...
    >
      {(data) =>
        data.providers.length === 0 ? (
          ...
          <>
            <SimpleTable
              headers={["Proveedor", "Llamadas", "Éxito", "Fallos", "Bloqueadas", "Límite", "Credencial", "p95", "Costo"]}
              align={{ 1: "right", 3: "right", 4: "right", 5: "right", 6: "right", 7: "right", 8: "right" }}
            >
              {data.providers.map((provider) => (
                <Tr key={provider.providerCode}>
                  <Td>
                    <p className="font-medium text-atlas-text">{provider.providerCode}</p>
                  ...
                  <Td right>{provider.p95LatencyMs === null ? "—" : `${formatNumber(provider.p95LatencyMs)} ms`}</Td>
                  <Td right>{formatNumber(provider.actualCost)}</Td>
                </Tr>
              ))}
            </SimpleTable>
            <p className="text-xs text-atlas-muted">
              Últimos {data.days} días · generado {formatDateTime(data.generatedAt)} · «Límite» son las llamadas que
              el proveedor rechazó por exceso de consultas; «Credencial», las que rechazó por autenticación.
            </p>
          </>
        )
      }
...
export function UsageReportView({ query }: Readonly<{ query: Parameters<typeof ReportShell<UsageReport>>[0]["query"] }>) {
  return (
    <ReportShell
      query={query}
      ...
      {(data) => (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
            <MetricCard label="Pedidas" value={data.summary.total} icon={PhoneCall} />
            <MetricCard label="Ejecutadas" value={data.summary.executed} icon={Play} tone="success" />
            <MetricCard
              label="Frenadas"
            ...
            <MetricCard
              ...
              icon={Database}
            />
            <MetricCard label="Costo estimado" value={formatNumber(data.summary.estimatedCost)} icon={Coins} />
            <MetricCard label="Costo real" value={formatNumber(data.summary.actualCost)} icon={Coins} />
          </div>
          <p className="text-xs text-atlas-muted">
            {data.providerCode === "ALL" ? "Todos los proveedores" : data.providerCode} · últimos {data.days} días ·
