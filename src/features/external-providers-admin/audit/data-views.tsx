...
import type { IdempotencyAudit, RetentionPreview, SanitizationAudit, Severity } from "../types";
import { EmptyGood, GateBanner, ReportShell, SimpleTable, Td, Tr } from "./report-shell";

function severidad(value: string) {
  return SEVERIDADES[value as Severity] ?? { label: value, tone: "default" as const };
}

export function IdempotencyAuditView({
  query,
}: Readonly<{ query: Parameters<typeof ReportShell<IdempotencyAudit>>[0]["query"] }>) {
  return (
    <ReportShell
      query={query}
      ...
      {(data) => (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <MetricCard label="Solicitudes revisadas" value={data.inspectedRequests} />
            <MetricCard
          ...
          {data.findings.length === 0 ? (
            <EmptyGood>No se encontró ninguna repetición en la ventana revisada.</EmptyGood>
          ) : (
            <SimpleTable headers={["Gravedad", "Qué pasó", "Veces", "Solicitudes"]} align={{ 2: "right" }}>
          ...
          <div className="rounded-xl border border-atlas-border bg-white p-4 shadow-subtle">
            <h4 className="mb-2 text-sm font-semibold text-atlas-text">Qué lo impide de entrada</h4>
...
export function RetentionPreviewView({
  query,
}: Readonly<{ query: Parameters<typeof ReportShell<RetentionPreview>>[0]["query"] }>) {
  return (
    <ReportShell
      query={query}
    ...
    >
      {(data) => {
        ...
        return (
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <MetricCard label="Candidatas a purga" value={data.candidateCount ?? candidatos.length} />
              <MetricCard label="Más antiguas que" value={`${data.olderThanDays ?? "—"} días`} />
            </div>
            {data.note ? <p className="text-sm text-atlas-muted">{data.note}</p> : null}
            {candidatos.length === 0 ? (
              <EmptyGood>No hay ninguna consulta lo bastante antigua para purgarse.</EmptyGood>
            ) : (
              <SimpleTable headers={["Solicitud", "Cliente", "Cuándo se pidió", "Cómo acabó", "Qué se haría"]}>
                  ...
                  <Tr key={candidato.requestId}>
                    ...
                    <Td>
                      {candidato.responseStatus ? (
                        <Badge tone={explainStatus(candidato.responseStatus).tone ?? "default"}>
...
export function SanitizationAuditView({
  query,
}: Readonly<{ query: Parameters<typeof ReportShell<SanitizationAudit>>[0]["query"] }>) {
  return (
    <ReportShell
      query={query}
    ...
    >
      {(data) => (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <MetricCard label="Respuestas revisadas" value={data.inspectedResponses} />
            <MetricCard
              label="Hallazgos"
              value={data.findings.length}
          ...
          {data.findings.length === 0 ? (
            ...
            <SimpleTable headers={["Gravedad", "Clave encontrada", "Respuesta", "Solicitud"]}>
              {data.findings.map((finding) => {
                const tono = severidad(finding.severity);
                return (
                  <Tr key={finding.responseId}>
                    ...
                    <Td>
                      ...
                      <p className="text-atlas-muted">{explainFinding(finding.code).summary}</p>
                    </Td>
