...
function Dato({ label, value }: Readonly<{ label: string; value: React.ReactNode }>) {
  return (
    <div className="min-w-0">
      <p className="text-xs uppercase tracking-[0.08em] text-atlas-muted">{label}</p>
      <p className="truncate text-sm font-medium tabular-nums text-atlas-text">{value}</p>
    </div>
  );
}
...
export function RequestResultCard({
  ...
  const estado = texto(result.responseStatus) ?? texto(anidado.responseStatus) ?? texto(result.status);
  const referencia = texto(result.providerReference) ?? texto(anidado.providerReference);
  const modo = texto(result.modeUsed) ?? texto(anidado.modeUsed);
  const latencia = numero(result.latencyMs) ?? numero(anidado.latencyMs);
  const requestId = texto(result.requestId) ?? texto(anidado.requestId) ?? texto(result.id);
  const explicacion = estado ? explainStatus(estado) : null;
  const malo = explicacion?.tone === "critical" || explicacion?.tone === "warning";
  ...
  return (
    <div className="space-y-3 rounded-xl border border-atlas-border bg-white p-4 shadow-subtle">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-atlas-muted">
            {explicacion?.summary ?? "La acción se ejecutó. El detalle está en los datos crudos."}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 border-t border-atlas-border pt-3 md:grid-cols-4">
        {estado ? (
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.08em] text-atlas-muted">Estado</p>
            <Badge tone={explicacion?.tone ?? "default"}>{explicacion?.label ?? estado}</Badge>
          </div>
        ) : null}
        {referencia ? <Dato label="Referencia" value={referencia} /> : null}
        {modo ? <Dato label="Modo usado" value={modo} /> : null}
        {latencia !== null ? <Dato label="Latencia" value={`${formatNumber(latencia)} ms`} /> : null}
      ...
      {abierto ? <JsonViewer title="Respuesta completa" value={result} /> : null}
    </div>
  );
}
