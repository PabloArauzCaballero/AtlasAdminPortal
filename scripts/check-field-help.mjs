 ...
 *
 * Hasta el 2026-09-15 los filtros de este portal eran `<select>` nativos con opciones de una sola
 * línea —«Último dato», «Recibos»— y campos con la etiqueta a secas. Lo que significaba cada cosa
 * vivía en un comentario del código, donde no lo lee quien usa el tablero. Esto impide que vuelva.
 *
 * Falla (salida 1, con `ruta:línea` y motivo) si en `src/`:
 *   1. un `<Field`, `<OptionSelect` o `<FieldLabel` no lleva `tooltip=`;
...
const ATOMOS = /<(Field|OptionSelect|FieldLabel)\b/g;
/** Los únicos archivos que pueden pintar la etiqueta y el desplegable a mano: los definen. */
const DEFINE_ETIQUETA =
  /shared\/components\/ui\/field-label\.tsx$|features\/qa-tutorials\/field-help\.tsx$/;
const DEFINE_SELECT =
  /shared\/components\/ui\/(option-select|input|form-select)\.tsx$|shared\/components\/data-table\/filter-bar\.tsx$/;
/** Los envoltorios que reciben el tooltip de quien los usa: el `<OptionSelect>` de dentro no lo lleva. */
const ENVUELVE_SELECT =
  /shared\/components\/ui\/(input|form-select)\.tsx$|shared\/components\/data-table\/filter-bar\.tsx$/;
...
function malRedactado(texto, etiqueta) {
  ...
  if (etiqueta && normalizar(texto) === normalizar(etiqueta))
    return "repite la etiqueta";
  return null;
}
...
for (const carpeta of CARPETAS) {
  ...
  for (const archivo of lista) {
    ...
    const exenta = (linea) =>
      EXCEPCION.test(lineas[linea - 1] ?? "") ||
      EXCEPCION.test(lineas[linea - 2] ?? "");
    const columnaDe = (indice) =>
    ...
    for (const [patron, motivo] of nativos) {
      ...
      for (const m of texto.matchAll(patron)) {
        ...
        if (motivo.startsWith("<label")) {
          ...
          const cuerpo = texto.slice(
            m.index,
            cierre === -1 ? m.index + 400 : cierre,
          );
          if (
            /type=["'{](checkbox|radio)/.test(cuerpo) ||
            /type="(checkbox|radio)"/.test(cuerpo)
          )
            continue;
        }
    ...
    for (const m of texto.matchAll(ATOMOS)) {
      ...
      camposRevisados += 1;
      const nombre =
        textoDe(etiqueta, "label") ?? textoDe(etiqueta, "name") ?? "?";
    ...
    for (const patron of opciones) {
      for (const m of texto.matchAll(patron)) {
        ...
        if (motivo) {
          hallazgos.push(
            `${rel}:${linea}  la description de «${nombre}» ${motivo}`,
          );
        }
      }
    }
